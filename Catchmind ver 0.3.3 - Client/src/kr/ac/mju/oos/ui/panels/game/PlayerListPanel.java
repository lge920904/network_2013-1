...
import kr.ac.mju.oos.ui.dialogs.*;
...
public class PlayerListPanel extends JPanel implements ActionListener{
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==button1 || e.getSource()==button2 || e.getSource()==button3){
			pld=new PlayerListDialog(e.getActionCommand()+"사용자 정보");// 아직 미완성
		}
	}
	...
	public void init(FrontController controller) {
		...
		label1.setIcon(player);
		label2.setIcon(player);
		label3.setIcon(player);
		this.setLayout(new GridLayout(3,1));
		this.add(new JPanel().add(label1));
		this.add(new JPanel().add(label2));
		this.add(new JPanel().add(label3));
		setVisible(true);
		//setSize(300,150);
		 */    
