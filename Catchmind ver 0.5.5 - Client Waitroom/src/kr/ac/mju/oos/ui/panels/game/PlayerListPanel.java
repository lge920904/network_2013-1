...
public class PlayerListPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == button1 || e.getSource() == button2
				|| e.getSource() == button3) {
			pld = new PlayerListDialog(e.getActionCommand() + "사용자 정보", "id",
					"rating");// 아직
			// 미완성
		}
	}
	...
	public void setText(String userID, int index) {
		if (index == 1) {
			button1.setText(userID);
		} else if (index == 2) {
			button2.setText(userID);
		} else if (index == 3) {
			button3.setText(userID);
		}
	}
